...
#load compiler
module load $1
#load zlib and not use the szlib,please antation.
module load $2
#load mpi
module load $3

module load $4
echo "Build Packages use $1 and $3"
...
./configure --prefix=/pkg/hdf5/1.8.10-parallel/$PRG_ENV/$MPI_PRG_ENV   CC=mpicc FC=mpif90   --enable-parallel  --enable-fortran  --with-zlib=$ZLIB_DIR --with-szlib=$SZIP_DIR --enable-hl  >>$LOG_DIR/$NAME/$PVERSION/$PRG_ENV/configure.log 2>&1
...
make -j 8  >>$LOG_DIR/$NAME/$PVERSION/${PRG_ENV}/make.log 2>&1

make check  >>$LOG_DIR/$NAME/$PVERSION/${PRG_ENV}/makecheck.log 2>&1
make install >>$LOG_DIR/$NAME/$PVERSION/$PRG_ENV/makeinstall.log 2>&1

echo "build OpenMPI  $NAME/$PVERSION  Done!"
